...
describe("presaleETH", function () {
	...
	beforeEach(async () => {
		...
		inkToken = (await MockPayToken.deploy()) as INK
		...
		presaleETH = (await PresaleETH.deploy(
			inkToken.address,
			startTime,
			endTime,
			vestingCliff
		...
		await inkToken.transfer(
			presaleETH.address,
			ethers.utils.parseEther("43750000000")
		)
		await inkToken.transfer(
			owner.address,
		...
		await inkToken
			.connect(owner)
			.approve(presaleETH.address, ethers.utils.parseEther("10000"))
	})

	it("test_setup", async function () {
		expect(await inkToken.balanceOf(presaleETH.address)).to.equal(
			ethers.utils.parseEther("43750000000")
		)
	...
	it("test_setPayRange_asUser_thenReverts", async () => {
		await expect(
			presaleETH.connect(user1).setPayRange(10, 100)
		).to.be.revertedWith("Ownable: caller is not the owner")
	})

	it("test_setPayRange_asOwner_givenInValidRange_thenReverts", async () => {
		await expect(
			presaleETH.connect(owner).setPayRange(100, 10)
		).to.be.revertedWith("set invalid range.")

		await expect(
			presaleETH.connect(owner).setPayRange(0, 100)
		).to.be.revertedWith("set invalid range.")
	})
	...
	it("test_setPrice_asUser_thenReverts", async () => {
		await expect(
			presaleETH.connect(user1).setPrice(10)
		).to.be.revertedWith("Ownable: caller is not the owner")
	})

	it("test_setPrice_asOwner_givenZeroPrice_thenReverts", async () => {
		await expect(presaleETH.connect(owner).setPrice(0)).to.be.revertedWith(
			"price is zero."
		)
	})
	...
	it("test_invest_afterPresaleClosed_thenReverts", async () => {
		...
		await ethers.provider.send("evm_mine", [])
		await expect(
			presaleETH.invest({ value: ethers.utils.parseEther("100") })
		).to.be.revertedWith("presale is closed.")
		await ethers.provider.send("evm_revert", [snapShot])
	})

	it("test_invest_givenLessThanMinPayAmount_thenReverts", async () => {
		await presaleETH.connect(owner)
		await presaleETH.setPayRange(
			ethers.utils.parseEther("10"),
			ethers.utils.parseEther("100")
		)

		await expect(
			presaleETH.invest({ value: ethers.utils.parseEther("3") })
		).to.be.revertedWith("fund is out of range.")
	})
	...
	it("test_invest_givenMoreThanMaxPayAmount_thenReverts", async () => {
		...
		await expect(
			presaleETH.invest({ value: ethers.utils.parseEther("200") })
		).to.be.revertedWith("fund is out of range.")
	})
	...
	it("test_invest_givenValidAmount_thenSuccessAndEmitFundsInvestedEvent", async () => {
		...
		const tx = await presaleETH
			.connect(user1)
			.invest({ value: ethers.utils.parseEther("20") })

		await expect(await presaleETH.balanceOfETH(user1.address)).to.equal(
			ethers.utils.parseEther("20")
		)
		await expect(
			await presaleETH.connect(owner).amountRaisedETH()
		).to.equal(ethers.utils.parseEther("20"))
		await expect(await presaleETH.balanceOfINK(user1.address)).to.equal(
			ethers.utils.parseEther("2000000000")
		)
		await expect(
			await presaleETH.connect(owner).amountRaisedINK()
		).to.equal(ethers.utils.parseEther("2000000000"))
		await expect(tx)
			.to.emit(presaleETH, "FundsInvested")
	...
	it("test_invest_reachGoal_thenSuccessAndEmitGoalReachedEvent", async () => {
		...
		const tx = await presaleETH
			.connect(user1)
			.invest({ value: ethers.utils.parseEther("4376") })

		await expect(await presaleETH.balanceOfETH(user1.address)).to.equal(
			ethers.utils.parseEther("4376")
		)
		await expect(
			await presaleETH.connect(owner).amountRaisedETH()
		).to.equal(ethers.utils.parseEther("4376"))
		await expect(await presaleETH.balanceOfINK(user1.address)).to.equal(
		...
		await expect(tx)
			.to.emit(presaleETH, "FundsInvested")
			.withArgs(user1.address, ethers.utils.parseEther("4376"))

		await expect(tx)
			.to.emit(presaleETH, "GoalReached")
	...
	it("test_getClamiableAmount_beforeCliff_thenReturnZero", async () => {
		...
		await presaleETH
			.connect(user2)
			.invest({ value: ethers.utils.parseEther("30") })
	...
	it("test_claim_claimableAmountIsZero_thenReverts", async () => {
		...
		await expect(presaleETH.connect(user2).claim()).to.be.revertedWith(
	...
	it("test_claim_afterPresaleClosed_thenSuccess", async () => {
		...
		assert.approximately(
			balance.div(ethers.utils.parseEther("1")).toNumber(),
			1000000000,
			100
		)
	...
	it("test_withdrawETH_asOwner_afterClosed_zeroPaidAmount_thenSuccess", async () => {
		...
		await expect(presaleETH.withdrawETH()).to.be.revertedWith(
			"withdraw ETH amount is zero."
		)
	...
	it("test_withdrawinkToken_asOwner_afterClosed_thenSuccess", async () => {
		const prevBalance = await inkToken.balanceOf(owner.address)
		const snapShot = await ethers.provider.send("evm_snapshot", [])
		...
		await expect(
			(await inkToken.balanceOf(presaleETH.address))
				.div(ethers.utils.parseEther("1"))
				.toNumber()
		).to.equal(2000000000)
		await expect(await inkToken.balanceOf(owner.address)).to.equal(
			prevBalance.add("41750000000000000000000000000")
		)
	...
	it("test_deposit_asOwner_givenZeroAmount_thenReverts", async () => {
		await expect(presaleETH.deposit(0)).to.be.revertedWith(
	...
	it("test_deposit_asOwner_givenNonZeroAmount_thenSuccess", async () => {
		...
		await expect(await inkToken.balanceOf(owner.address)).to.equal(
			prevBalance.sub(ethers.utils.parseEther("3000"))
		)
		await expect(await inkToken.balanceOf(presaleETH.address)).to.equal(
			ethers.utils.parseEther("43750003000")
		)
	})
})
